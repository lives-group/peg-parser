  ...
  (let ([types (infer-types grammar)]
    ...
    (datum->syntax
     ...
     `(module peg-parser racket
         (require "../peg-wf.rkt")
        ,(display "Well-typed? " )
        ,(displayln (satisfied? types))
        ,(displayln `"---- Constraint List ----" )
        ,(displayln (ctx-constr-toString types))
        ,(displayln `"----Unsolved Nts ----" )
        ,(displayln (unsolved-nts  types))
        ,(displayln `"---- Substitution ----" )
        ,(displayln (ctx-subst-toString types))
        ,(displayln `"----Type Env ----" )
        ,(displayln (ctx-tyEnv-toString  types))
        ,(displayln `"----Start type ----" )
        ,(displayln (ty->string (get-start-exp-ty types)))
        ))))
  )